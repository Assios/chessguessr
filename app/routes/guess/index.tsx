...
const getArrayOfRandomNumbers = (nums, arrayLength) => {
  const numbers = [...Array(nums).keys()];
  numbers.sort(() => Math.random() - 0.5);
  return numbers.slice(0, arrayLength);
};

const getArrayOfRandomNumbersExcluding = (nums, arrayLength, excludingNum) => {
  const numbers = [...Array(nums).keys()];
  const numsExcluded = numbers.filter((i) => i !== excludingNum);
  numsExcluded.sort(() => Math.random() - 0.5);
  return numsExcluded.slice(0, arrayLength);
};
...
const index = () => {
  ...
  const makeQuiz = (numQuestions: number) => {
    ...
    const questionIndices = getArrayOfRandomNumbers(
      allQuesitons.length,
      numQuestions
    );
    ...
    for (let i = 0; i < questionIndices.length; i++) {
      ...
      const optionsIndices = getArrayOfRandomNumbersExcluding(
        allQuesitons.length,
        3,
